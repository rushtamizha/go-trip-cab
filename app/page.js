...
import { Phone, MessageCircle, Users, MapPin, Clock, IndianRupee, ChevronLeft, ChevronRight, Plane, Car, Navigation, Map, Star, Shield, Headphones, CreditCard, Info, AlertCircle } from 'lucide-react';
import { company, heroImages, airportTaxis, airportPolicy, localTaxis, localNotes, outstationTaxis, outstationTerms, tourPackages, tourCategories, getTourMinPrice } from '@/lib/data';
  ...
  return (
    <div className="relative w-full h-[400px] md:h-[500px] overflow-hidden">
      ...
      <div className="absolute z-10 flex hidden gap-2 bottom-4 right-6">
        {heroImages.map((_, i) => (
          <button key={i} onClick={() => setCurrent(i)} className={`w-3 h-3 rounded-full transition-all ${i === current ? 'bg-orange-500 w-8' : 'bg-white/60'}`} />
        ))}
      </div>
    ...
    <div onClick={() => onClick(cab.id)} className="overflow-hidden transition-all duration-300 bg-white border border-orange-100 shadow-sm cursor-pointer rounded-2xl hover:shadow-md group">
      ...
      <div className="p-5">
        ...
        <div className="flex gap-2">
          ...
          <a href={`tel:${company.phone}`} onClick={(e) => e.stopPropagation()} className="flex items-center justify-center transition-colors bg-blue-100 w-11 h-11 hover:bg-blue-200 rounded-xl"><Phone className="w-4 h-4 text-blue-600" /></a>
          <a href={`https://wa.me/${company.whatsapp}`} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()} className="flex items-center justify-center transition-colors bg-green-100 w-11 h-11 hover:bg-green-200 rounded-xl"><MessageCircle className="w-4 h-4 text-green-600" /></a>
        </div>
      </div>
    </div>
  );
...
function TourCard({ tour, onClick }) {
  ...
  return (
    <div onClick={() => onClick(tour.slug)} className="overflow-hidden transition-all duration-300 bg-white border border-orange-100 shadow-sm cursor-pointer rounded-2xl hover:shadow-md group">
...
function Features() {
  const features = [
    { icon: Shield, title: 'Safe & Reliable', desc: 'Verified drivers & well-maintained vehicles' },
    { icon: IndianRupee, title: 'Best Prices', desc: 'Transparent pricing with no hidden charges' },
    { icon: Clock, title: '24/7 Available', desc: 'Round the clock service, every day' },
    { icon: CreditCard, title: 'Easy Payment', desc: 'Cash, UPI, GPay, PhonePe, Paytm' },
  ];
  return (
        ...
        <div className="grid grid-cols-2 gap-6 md:grid-cols-4">
          {features.map((f, i) => (
            <div key={i} className="text-center">
              <div className="inline-flex items-center justify-center mb-3 bg-orange-500 w-14 h-14 rounded-2xl"><f.icon className="text-white w-7 h-7" /></div>
    ...
    <nav className="sticky top-0 z-50 bg-white border-b border-orange-100 shadow-sm">
      <div className="flex items-center justify-between px-4 py-3 mx-auto max-w-7xl">
        ...
        <div className="flex items-center gap-3">
          <a href={`tel:${company.phone}`} className="items-center hidden gap-2 px-4 py-2 text-sm font-medium text-orange-600 transition-colors md:flex bg-orange-50 hover:bg-orange-100 rounded-xl"><Phone className="w-4 h-4" />{company.phoneDisplay}</a>
          <a href={`https://wa.me/${company.whatsapp}`} target="_blank" rel="noreferrer" className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white transition-colors bg-green-500 hover:bg-green-600 rounded-xl"><MessageCircle className="w-4 h-4" /><span className="hidden md:inline">WhatsApp</span></a>
        </div>
      </div>
    </nav>
  );
  ...
  return (
    <div className="p-5 mt-6 bg-orange-50 rounded-2xl">
      <button onClick={() => setOpen(!open)} className="flex items-center justify-between w-full text-left">
        <div className="flex items-center gap-2"><Info className="w-5 h-5 text-orange-500" /><h3 className="font-bold text-gray-800">{title}</h3></div>
      ...
      <div className="px-4 mx-auto max-w-7xl">
        <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm text-gray-400"><li>Airport Taxi</li><li>Outstation Cabs</li><li>Local Taxi Rental</li><li>Tour Packages</li></ul>
          </div>
          <div>
            <h4 className="mb-4 font-bold text-orange-400">Contact Us</h4>
            <div className="space-y-3">
              <a href={`tel:${company.phone}`} className="flex items-center gap-2 text-sm text-gray-400 transition-colors hover:text-orange-400"><Phone className="w-4 h-4" />{company.phoneDisplay}</a>
              <a href={`https://wa.me/${company.whatsapp}`} className="flex items-center gap-2 text-sm text-gray-400 transition-colors hover:text-green-400"><MessageCircle className="w-4 h-4" />WhatsApp Us</a>
...
function FloatingWhatsApp() {
  return (
    <a href={`https://wa.me/${company.whatsapp}?text=Hi, I want to book a cab from Bangalore`} target="_blank" rel="noreferrer" className="fixed z-50 p-4 text-white transition-all bg-green-500 rounded-full shadow-2xl bottom-6 right-6 hover:bg-green-600 hover:scale-110">
...
export default function App() {
  ...
  const handleTourClick = useCallback((slug) => {
    router.push(`/tour/${slug}`);
  }, [router]);

  const handleAirportClick = useCallback((id) => {
    router.push(`/airport/${id}`);
  }, [router]);

  const handleOutstationClick = useCallback((id) => {
    router.push(`/outstation/${id}`);
  }, [router]);

  const handleLocalClick = useCallback((id) => {
    router.push(`/local/${id}`);
  }, [router]);

  const filteredTours = tourFilter === 'all'
    ? tourPackages
    : tourPackages.filter(t => t.type === tourFilter || t.category === tourFilter);

  const sectionTitles = {
    airport: { title: 'Book Airport Taxi in Bangalore', subtitle: 'Choose from the wide range of Cab/Taxi with our airport taxi booking in Bangalore' },
    outstation: { title: 'Book Outstation Taxi Hire from Bangalore', subtitle: 'Choose from the wide range of outstation taxi booking in Bangalore' },
    local: { title: 'Best Bangalore Local Taxi Hire Service', subtitle: 'Choose from the wide range of Cab/Taxi with our local taxi booking in Bangalore' },
    tour: { title: 'Tour Packages from Bangalore', subtitle: 'Explore handpicked destinations with our all-inclusive cab packages' },
  };
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="px-4 py-8 mx-auto max-w-7xl">
        ...
        <div className="flex justify-center mb-6">
          <div className="inline-flex bg-orange-50 rounded-2xl p-1.5 gap-1 flex-wrap justify-center">
            {TABS.map((tab) => {
              const Icon = tab.icon;
              return (
                <button key={tab.id} onClick={() => { setActiveTab(tab.id); setTourFilter('all'); }}
                  className={`flex items-center gap-2 px-4 md:px-5 py-3 rounded-xl text-sm font-semibold transition-all duration-300 ${activeTab === tab.id ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/30' : 'text-gray-600 hover:text-orange-600 hover:bg-orange-100'}`}>
        ...
        <div className="mb-6 text-center">
          <h2 className="mb-2 text-2xl font-bold text-gray-900 md:text-3xl">{sectionTitles[activeTab].title}</h2>
          <p className="max-w-2xl mx-auto text-sm text-gray-500">{sectionTitles[activeTab].subtitle}</p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {activeTab === 'airport' && airportTaxis.map((cab) => <AirportCard key={cab.id} cab={cab} onClick={handleAirportClick} />)}
          {activeTab === 'outstation' && outstationTaxis.map((cab) => <OutstationCard key={cab.id} cab={cab} onClick={handleOutstationClick} />)}
          {activeTab === 'local' && localTaxis.map((cab) => <LocalCard key={cab.id} cab={cab} onClick={handleLocalClick} />)}
          {activeTab === 'tour' && filteredTours.map((tour) => <TourCard key={tour.id} tour={tour} onClick={handleTourClick} />)}
        </div>
        ...
        {activeTab === 'airport' && <PolicySection title="Airport Taxi Policy" items={airportPolicy} />}

        {/* Policy notes for local */}
        {activeTab === 'local' && <PolicySection title="Local Rental Notes" items={localNotes} />}

        {/* Policy notes for outstation */}
        {activeTab === 'outstation' && (
          <PolicySection title="Outstation Terms & Conditions" items={[
